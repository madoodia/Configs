...
vec3 setColor(const Ray &ray, Hitable *world, int depth) {
        ...
        if (world->hit(ray, 0.001, FLT_MAX, record)) {
                Ray scattered;
                vec3 attenuation;
                if (depth < 50 && record.matPtr->scatter(
                                      ray, record, attenuation, scattered)) {
                        return attenuation * setColor(scattered, world,
                                                      depth + 1); // recursive
                } else {
                        return vec3(0, 0, 0);
                }
...
Hitable *randomScene() {
        ...
        for (int a = -11; a < 11; a++) {
                for (int b = -11; b < 11; b++) {
                        ...
                        vec3 center(a + 0.9 * randomDouble(), 0.2,
                                    b + 0.9 * randomDouble());
...
int main() {
        ...
        // inline functions and regular functions.
        {
                ...
                Camera cam(lookFrom, lookAt, vec3(0, 1, 0), 20,
                           float(nx) / float(ny), aperture, dof);
                for (int j = ny - 1; j >= 0; j--) {
                        for (int i = 0; i < nx; i++) {
                                vec3 col(0.0, 0.0, 0.0);
                                for (int s = 0; s < ns; s++) {
                                        float u = float(i + randomDouble()) /
                                                  float(nx);
                                        float v = float(j + randomDouble()) /
                                                  float(ny);
                                ...
                                col /= float(ns);
                                col =
                                    vec3(sqrt(col.x), sqrt(col.y), sqrt(col.z));
