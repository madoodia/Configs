...
call "%VCVARS_LOCATION%/vcvarsall.bat" x64 -vcvars_ver=14.33
...
cmake -G "NMake Makefiles" -DCMAKE_BUILD_TYPE=Debug -DCMAKE_PREFIX_PATH=%EXTRA_PREFIX_PATH% "%ROOT%"
cmake --build .
...
if exist "deploy" (
    echo ------------------------------------------------------------------------------------------------
    echo ------------ Copy Zolf.exe and glew.dll and shaders and textures to deploy folder --------------
    echo ------------------------------------------------------------------------------------------------
    call %WIN_DIR%envVars.bat
